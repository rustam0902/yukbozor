...
export function getDistrictDisplayName(regionName: string, districtName: string, language: 'ru' | 'uz' = 'ru'): string {
  const region = getRegionByName(regionName);
...
export function formatPrice(price: number | string | null | undefined, language: 'ru' | 'uz' = 'ru'): string {
  if (price === null || price === undefined) return language === 'ru' ? '0 сум' : '0 so\'m';
  const numPrice = typeof price === 'string' ? parseFloat(price) : price;
  if (isNaN(numPrice)) return language === 'ru' ? '0 сум' : '0 so\'m';
  const formatted = numPrice.toLocaleString('ru-RU');
  return language === 'ru' ? `${formatted} сум` : `${formatted} so'm`;
}

// Format date
export function formatDate(date: string | Date | null | undefined, language: 'ru' | 'uz' = 'ru'): string {
  if (!date) return '-';
...
export function getRegionName(regionName: string | null | undefined, language: 'ru' | 'uz' = 'ru'): string {
  if (!regionName) return '-';
  return getRegionDisplayName(regionName, language);
}

// Transport type names
export function getTransportTypeName(type: string | null | undefined, language: 'ru' | 'uz' = 'ru'): string {
  if (!type) return '-';
  const types: Record<string, { ru: string; uz: string }> = {
    labo: { ru: 'Лабо', uz: 'Labo' },
    bongo: { ru: 'Бонго', uz: 'Bongo' },
    furgon: { ru: 'Фургон', uz: 'Furgon' },
    isuzu5: { ru: 'Исузу-5', uz: 'Isuzu-5' },
    isuzu10: { ru: 'Исузу-10', uz: 'Isuzu-10' },
    gruzovik: { ru: 'Грузовик', uz: 'Yuk avtomobili' },
    fura_tent: { ru: 'Фура тент', uz: 'Fura tent' },
    fura_ref: { ru: 'Фура рефрижератор', uz: 'Fura muzlatgich' },
    paravoz: { ru: 'Паравоз', uz: 'Paravoz' },
    shalanda: { ru: 'Шаланда', uz: 'Shalanda' },
    traller: { ru: 'Трейлер', uz: 'Treyler' },
    tonar: { ru: 'Тонар', uz: 'Tonar' },
    benzovoz: { ru: 'Бензовоз', uz: 'Benzovoz' },
    konteynerovoz: { ru: 'Контейнеровоз', uz: 'Konteyner tashuvchi' },
    other: { ru: 'Другой', uz: 'Boshqa' },
  };
  return types[type]?.[language] || type;
}

// Format date and time
export function formatDateTime(date: string | Date | null | undefined, language: 'ru' | 'uz' = 'ru'): string {
  if (!date) return '-';
  const d = typeof date === 'string' ? new Date(date) : date;
  if (isNaN(d.getTime())) return '-';
  const dateStr = d.toLocaleDateString(language === 'ru' ? 'ru-RU' : 'uz-UZ', {
    day: '2-digit',
    month: '2-digit',
    year: 'numeric',
  });
...
// Order status names
export function getOrderStatusName(status: string | null | undefined, language: 'ru' | 'uz' = 'ru', deletedAt?: string | null): string {
  if (deletedAt) return language === 'ru' ? 'Удалён' : 'O\'chirilgan';
  if (!status) return '-';
...
// Order status colors
export function getOrderStatusColor(status: string | null | undefined, deletedAt?: string | null): string {
  if (deletedAt) return '#6B7280';
...
// Contract status names
export function getContractStatusName(status: string | null | undefined, language: 'ru' | 'uz' = 'ru'): string {
  if (!status) return '-';
  const statuses: Record<string, { ru: string; uz: string }> = {
    pending: { ru: 'Ожидает', uz: 'Kutilmoqda' },
    active: { ru: 'Активный', uz: 'Faol' },
    in_transit: { ru: 'В пути', uz: 'Yo\'lda' },
    delivered: { ru: 'Доставлен', uz: 'Yetkazildi' },
    completed: { ru: 'Завершён', uz: 'Yakunlangan' },
    cancelled: { ru: 'Отменён', uz: 'Bekor qilingan' },
    awaiting_prepayment: { ru: 'Ожидает предоплаты', uz: 'To\'lov kutilmoqda' },
    fully_signed: { ru: 'Подписан сторонами', uz: 'Imzolangan' },
    concluded: { ru: 'Заключён', uz: 'Tuzilgan' },
    signed: { ru: 'Подписан', uz: 'Imzolangan' },
    disputed: { ru: 'Спор', uz: 'Nizo' },
    terminated: { ru: 'Расторгнут', uz: 'Bekor qilingan' },
    draft: { ru: 'Черновик', uz: 'Qoralama' },
    prepayment_made: { ru: 'Предоплата внесена', uz: 'To\'lov amalga oshirildi' },
    pending_carrier_signature: { ru: 'Ожидает подписи перевозчика', uz: 'Tashuvchi imzosi kutilmoqda' },
    pending_customer_signature: { ru: 'Ожидает подписи клиента', uz: 'Mijoz imzosi kutilmoqda' },
    signed_by_customer: { ru: 'Подписан клиентом', uz: 'Mijoz imzolagan' },
    signed_by_carrier: { ru: 'Подписан перевозчиком', uz: 'Tashuvchi imzolagan' },
    awaiting_completion_confirmation: { ru: 'Ожидает завершения', uz: 'Yakunlanish kutilmoqda' },
  };
  return statuses[status]?.[language] || status;
}

// Contract status colors
export function getContractStatusColor(status: string | null | undefined): string {
  const colors: Record<string, string> = {
    pending: '#F59E0B',
    active: '#3B82F6',
    in_transit: '#8B5CF6',
    ...
    terminated: '#6B7280',
    draft: '#9CA3AF',
    prepayment_made: '#10B981',
    pending_carrier_signature: '#F59E0B',
    pending_customer_signature: '#F59E0B',
    signed_by_customer: '#3B82F6',
    signed_by_carrier: '#3B82F6',
    awaiting_completion_confirmation: '#F59E0B',
  };
  return colors[status || ''] || '#6B7280';
}

// Transport types array for filters
export const transportTypes = [
  { name: 'labo', nameRu: 'Лабо', nameUz: 'Labo' },
  { name: 'bongo', nameRu: 'Бонго', nameUz: 'Bongo' },
  { name: 'furgon', nameRu: 'Фургон', nameUz: 'Furgon' },
  { name: 'isuzu5', nameRu: 'Исузу-5', nameUz: 'Isuzu-5' },
  { name: 'isuzu10', nameRu: 'Исузу-10', nameUz: 'Isuzu-10' },
  { name: 'gruzovik', nameRu: 'Грузовик', nameUz: 'Yuk avtomobili' },
  { name: 'fura_tent', nameRu: 'Фура тент', nameUz: 'Fura tent' },
  { name: 'fura_ref', nameRu: 'Фура рефрижератор', nameUz: 'Fura muzlatgich' },
  { name: 'paravoz', nameRu: 'Паравоз', nameUz: 'Paravoz' },
  { name: 'shalanda', nameRu: 'Шаланда', nameUz: 'Shalanda' },
  { name: 'traller', nameRu: 'Трейлер', nameUz: 'Treyler' },
  { name: 'tonar', nameRu: 'Тонар', nameUz: 'Tonar' },
  { name: 'benzovoz', nameRu: 'Бензовоз', nameUz: 'Benzovoz' },
  { name: 'konteynerovoz', nameRu: 'Контейнеровоз', nameUz: 'Konteyner tashuvchi' },
  { name: 'other', nameRu: 'Другой', nameUz: 'Boshqa' },
];

// Get district display name
export function getDistrictName(regionName: string | null | undefined, districtName: string | null | undefined, language: 'ru' | 'uz' = 'ru'): string {
  if (!regionName || !districtName) return districtName || '';
  ...
  return language === 'uz' ? district.nameUz : district.nameRu;
}
